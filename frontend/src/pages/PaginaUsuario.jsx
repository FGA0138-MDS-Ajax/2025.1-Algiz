...
export default function PaginaUsuario() {
  ...
  // Defina claramente se é o próprio usuário ou outro perfil
  const isPerfilProprio = usuarioLogado && Number(idUsuario) === usuarioLogado.id;
  
  // O usuário só está "logado" no contexto do perfil se estiver vendo seu próprio perfil
  // e não estiver no modo de visualização pública
  const isUsuarioLogado = isPerfilProprio && !visualizandoPublico;
  
  // O botão de alternar visualização pública só deve aparecer no próprio perfil
  const mostrarBotaoVisualizacao = isPerfilProprio;
  ...
  useEffect(() => {
    async function fetchUsuario() {
      ...
      try {
        // Verificar se estamos acessando perfil de outro usuário
        const acessandoOutroPerfil = usuarioLogado && Number(idUsuario) !== usuarioLogado.id;
        
        // Para depuração
        console.log("Acessando perfil com ID:", idUsuario);
        console.log("Usuário logado ID:", usuarioLogado?.id);
        console.log("É outro perfil?", acessandoOutroPerfil);
        
        let url, options;
        
        // Use a rota pública em três casos:
        // 1. Não há usuário logado
        // 2. Estamos visualizando no modo público
        // 3. Estamos acessando o perfil de outro usuário
        if (!usuarioLogado || visualizandoPublico || acessandoOutroPerfil) {
          console.log("Usando rota pública");
          url = `http://localhost:3001/api/users/${idUsuario}/public`;
          options = { 
            headers: { 
              "Content-Type": "application/json" 
            },
            // Não enviar credenciais para rotas públicas
            credentials: 'omit'
          };
        } else {
          console.log("Usando rota privada (autenticada)");
        ...
        // Se estamos acessando o perfil de outro usuário, forçar modo de visualização pública
        if (acessandoOutroPerfil && !visualizandoPublico) {
          console.log("Forçando modo visualização pública para perfil de outro usuário");
          setVisualizandoPublico(true);
        }
      } catch (err) {
  ...
  // Handler de alternância de visualização
  const handleToggleVisualizacaoPublica = () => {
    // Só permitir alternar se for o próprio perfil
    if (isPerfilProprio) {
    ...
    <div className="bg-green-50 min-h-screen py-2 relative pt-16">
      <div className="container mx-auto flex flex-col lg:flex-row gap-6 max-w-6xl mt-8">
        ...
        <section className="flex-1 flex flex-col gap-2">
          ...
          {/* Mostrar os componentes com base em se é usuário logado ou não */}
          {isUsuarioLogado ? (
          ...
          {/* Salvos sempre aparece, mas passa flag de público quando necessário */}
          <Salvos usuario={usuario} isPublic={!isUsuarioLogado} />
        </section>

        {/* Sidebar de sugestões */}
        <aside className="w-full lg:w-80 mt-6 lg:mt-0">
          {/* Configurações e Empresas Associadas aparecem APENAS quando o usuário está logado vendo seu próprio perfil */}
          {isUsuarioLogado && (
            <>
          ...
          {/* Sugestões empresas sempre aparecem */}
          <SugestoesEmpresas
            sugestoes={sugestoes}
      ...
      {/* O botão de sair da visualização pública só aparece quando o usuário
          está vendo seu próprio perfil em modo público */}
      {isPerfilProprio && visualizandoPublico && (
        <button
